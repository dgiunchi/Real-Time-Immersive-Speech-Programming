...
Clean CentOS 9 server for this project.
...
Install Node.js 20+ from NodeSource or your standard server image. Then:

```bash
cd "/path/to/Real-Time-Immersive-Speech-Programming/Server linux"
npm install
python3 -m venv ./venv
...
export STT_HTTP_URL="http://130.136.2.161:50101/stt/transcribe"
```
...
For `systemd`, set `WorkingDirectory` to this folder and `ExecStart=/usr/bin/npm start`.
...
```bash
curl http://130.136.2.161:50101/health
npm run check
```
...
Do not put API keys in `config.json`. Use `OPENAI_API_KEY`.

If remote Unity cannot connect, check server firewall, router/NAT, and `Unity/Assets/Demos/Server.asset` address/port.
...
If Ubiq fails with `EADDRINUSE: ... 50000`, another server is already running. Stop old `node app.js` / `npm start` process, then run this server again.
