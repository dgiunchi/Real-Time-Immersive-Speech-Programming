...
namespace DreamCodeVR2.ExperimentalAuthoring
{
    public class DreamCodeVR2ResearcherControlClient : MonoBehaviour
    {
        ...
        public void StartSession(ExperimentCondition condition, string peerUUID, Action<Response> done) => StartCoroutine(Request("POST", "/api/authoring/dev/session/start", JsonUtility.ToJson(new RequestBody{condition=ServerCondition(condition),peerUUID=peerUUID}), peerUUID, done));
        public void RestartSession(ExperimentCondition condition, string peerUUID, Action<Response> done) => StartCoroutine(Request("POST", "/api/authoring/dev/session/restart", JsonUtility.ToJson(new RequestBody{condition=ServerCondition(condition),peerUUID=peerUUID}), peerUUID, done));
