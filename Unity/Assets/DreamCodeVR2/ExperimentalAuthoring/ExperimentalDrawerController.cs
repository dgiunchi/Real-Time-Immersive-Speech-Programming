...
namespace DreamCodeVR2.ExperimentalAuthoring
{
    public class ExperimentalDrawerController : MonoBehaviour
    {
        ...
        {
            if (!TryGetTarget(requestedOpen, out var target, out error)) { Log("DRAWER_MOTION_CONFIGURATION_ERROR", requestedOpen, error, transform.position, null); return false; }
            if (motion != null) { StopCoroutine(motion); motion = null; MotionInterrupted?.Invoke(requestedOpen); Log("DRAWER_MOTION_INTERRUPTED", requestedOpen, null, transform.position, target.position); }
        ...
        {
            var fromPosition = transform.position; var fromRotation = transform.rotation; var safeDuration = Mathf.Max(.01f, duration); var elapsed = 0f;
            while (elapsed < safeDuration) { elapsed += Time.deltaTime; var t = Mathf.SmoothStep(0f, 1f, Mathf.Clamp01(elapsed / safeDuration)); SetPose(Vector3.LerpUnclamped(fromPosition, target.position, t), Quaternion.Slerp(fromRotation, target.rotation, t)); yield return null; }
            SetPose(target); Complete(requestedOpen, target);
        }

        private void Complete(bool requestedOpen, Transform target)
        {
            motion = null; var changed=IsOpen!=requestedOpen; IsOpen = requestedOpen;
        ...
        {
            var item=GetComponent<AIEditableObject>();var id=item?.objectId;
            ...
            reporter?.DrawerStateChanged(item,semantic.state);
        ...
        private void SetPose(Vector3 position, Quaternion rotation)
        {
            ...
            transform.position = position; if (applyAnchorRotation) transform.rotation = rotation;
        }
        private void Log(string eventName, bool requestedOpen, string error, Vector3 source, Vector3? target) => DreamCodeVR2ClientLogger.Event("drawer", eventName, error, new { object_id = GetComponent<AIEditableObject>()?.objectId, source_position = source, target_position = target, motion_duration = duration, requested_state = requestedOpen ? "open" : "closed" });
        private void OnDrawGizmosSelected(){if (!closedAnchor || !openAnchor) return;Gizmos.color = Color.cyan; Gizmos.DrawWireSphere(closedAnchor.position, .035f);Gizmos.color = Color.green; Gizmos.DrawWireSphere(openAnchor.position, .035f);Gizmos.color = Color.yellow; Gizmos.DrawLine(closedAnchor.position, openAnchor.position);}
    }
}
