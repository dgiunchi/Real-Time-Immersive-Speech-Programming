...
namespace DreamCodeVR2.ExperimentalAuthoring
{
    public class AuthoringActionExecutor : MonoBehaviour
    {
        ...
        public AuthoringExecutionResult Execute(AuthoringAction action)
        {
            ...
            IsExecuting = true;
            try { var result = ValidateAndApply(action); if (result.success) processedActionIds.Add(action.actionId); ActionFinished?.Invoke(result); sceneContextTransmitter?.SendSceneContextSnapshot(result.success ? "authoring action" : "authoring failure"); return result; }
