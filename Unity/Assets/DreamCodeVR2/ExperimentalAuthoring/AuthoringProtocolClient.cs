...
namespace DreamCodeVR2.ExperimentalAuthoring
{
    public class AuthoringProtocolClient : MonoBehaviour
    {
        ...
        public void ProcessMessage(ReferenceCountedSceneGraphMessage data)
        {
            ...
            else if(envelope.type=="PredefinedCommandRejected") { DreamCodeVR2ClientLogger.Warn("c1","PREDEFINED_COMMAND_REJECTED_BY_SERVER",envelope.reason,new { command_id=envelope.command_id,interpretation=envelope.interpretation,target_object_id=envelope.target_object_id });var cancelled=proposalPresenter?.DismissRejectedPredefinedProposal(envelope.command_id)??false;if(!cancelled)proposalPresenter?.ShowC1Failure(envelope.reason,"server_rejection",envelope.command_id); telemetry?.Log("server_status",null,envelope.command_id,false); }
            else if(envelope.type=="PredefinedCommandAck"&&string.Equals(envelope.status,"failed",StringComparison.OrdinalIgnoreCase)) proposalPresenter?.ShowC1Failure(envelope.reason??envelope.detail,"server_ack",envelope.command_id);
        ...
        private void HandleNextTaskActivation(string taskId)
        {
            ...
            LogActivationCorrelation(taskId,generatedSeen,false);
            if(pendingFixedQuestInstance!=null)questInstanceController.Apply(pendingFixedQuestInstance);else questInstanceController.ActivateServerTask(pendingFixedTask);
