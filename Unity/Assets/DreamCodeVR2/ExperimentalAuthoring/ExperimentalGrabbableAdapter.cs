...
namespace DreamCodeVR2.ExperimentalAuthoring
{
    ...
    public class ExperimentalGrabbableAdapter : MonoBehaviour, IGraspable
    {
        ...
        public void Release(Hand proxy) { if(!IsHeld)return;var wasHeld=IsHeld; IsHeld=false; transform.SetParent(originalParent,true); if(body)body.isKinematic=originalKinematic; var id=GetComponent<AIEditableObject>()?.objectId; eventBus?.Publish(QuestEventType.ObjectDropped,id);Released?.Invoke(this,id,wasHeld);sceneContext?.SendSceneContextSnapshot("object released"); }
    }
}
