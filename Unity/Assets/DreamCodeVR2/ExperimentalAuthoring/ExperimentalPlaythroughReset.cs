...
namespace DreamCodeVR2.ExperimentalAuthoring
{
    ...
    {
        private class Snapshot { public AIEditableObject editable; public Vector3 position; public Quaternion rotation; public Vector3 scale; public bool active; public bool kinematic; public bool gravity; public bool[] colliderEnabled; public Renderer[] renderers; public Color[] colors; public bool[] colorReadable; public string semanticState; public bool grabbable; public bool movable; public bool interactable; public bool adapterGrabbable; }
        ...
        { snapshots.Clear(); foreach(var e in FindObjectsByType<AIEditableObject>(FindObjectsInactive.Include,FindObjectsSortMode.None)){var body=e.GetComponent<Rigidbody>();var cs=e.GetComponentsInChildren<Collider>(true);var rs=e.GetComponentsInChildren<Renderer>(true);var afford=e.GetComponent<AuthoringAffordanceState>();var adapter=e.GetComponent<ExperimentalGrabbableAdapter>();var s=new Snapshot{editable=e,position=e.transform.position,rotation=e.transform.rotation,scale=e.transform.localScale,active=e.gameObject.activeSelf,kinematic=body&&body.isKinematic,gravity=body&&body.useGravity,colliderEnabled=new bool[cs.Length],renderers=rs,colors=new Color[rs.Length],colorReadable=new bool[rs.Length],semanticState=e.GetComponent<AuthoringSemanticState>()?.state,grabbable=afford&&afford.grabbable,movable=afford&&afford.movable,interactable=afford&&afford.interactable,adapterGrabbable=adapter&&adapter.grabbable};for(int i=0;i<cs.Length;i++)s.colliderEnabled[i]=cs[i].enabled;for(int i=0;i<rs.Length;i++)s.colorReadable[i]=TryReadColor(rs[i]?rs[i].material:null,out s.colors[i]);snapshots.Add(s);} }
        public void ResetExperimentalPlaythrough()
        {
            FindFirstObjectByType<DreamCodeVR2.UI.DreamCodeVRAuthoringUIController>()?.ClearParticipantCommandFeedback();
            FindFirstObjectByType<QuestInstanceController>()?.ClearC1QuestSphere();
            ...
            foreach(var lockController in FindObjectsByType<QuestLockController>(FindObjectsInactive.Include,FindObjectsSortMode.None)) lockController.ResetLocked();
            foreach(var door in FindObjectsByType<QuestDoorController>(FindObjectsInactive.Include,FindObjectsSortMode.None)) door.TryClose(out _);
            foreach(var painting in FindObjectsByType<QuestPaintingController>(FindObjectsInactive.Include,FindObjectsSortMode.None)) painting.ResetCrooked();
            foreach(var lamp in FindObjectsByType<QuestLampController>(FindObjectsInactive.Include,FindObjectsSortMode.None)) lamp.SetLampState(false);
            foreach(var lamp in FindObjectsByType<QuestLampController>(FindObjectsInactive.Include,FindObjectsSortMode.None)) lamp.SetColorProfile("default");
        ...
        private static bool TryReadColor(Material m,out Color color){if(m&&m.HasProperty("_BaseColor")){color=m.GetColor("_BaseColor");return true;}if(m&&m.HasProperty("_Color")){color=m.GetColor("_Color");return true;}color=Color.white;return false;}
