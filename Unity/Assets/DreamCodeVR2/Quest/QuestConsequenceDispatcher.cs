    ...
    public sealed class QuestConsequenceDispatcher : MonoBehaviour
    {
        ...
        private IEnumerator ApplyRoutine(QuestConsequenceInstruction i)
        {
            ...
            switch(type)
            {
                ...
                case "SET_OBJECT_VISIBILITY": {if(!target)error="target_not_found";else {var visible=string.Equals(Value(i,"visible","state"),"true",StringComparison.OrdinalIgnoreCase)||string.Equals(Value(i,"visible","state"),"visible",StringComparison.OrdinalIgnoreCase);target.gameObject.SetActive(visible);state=visible?"visible":"hidden";}break;}
                ...
                case "REVEAL_OBJECT_IN_CONTAINER": {if(!target)error="target_not_found";else {var anchor=FindAnchor(i.containerId??Value(i,"anchor_id"));if(!anchor)error="invalid_container";else {target.transform.SetParent(anchor.transform,true);target.gameObject.SetActive(true);if((target.objectId??string.Empty).StartsWith("key",StringComparison.OrdinalIgnoreCase))KeyPoseNormalizer.Normalize(target,"consequence_reveal",anchor.transform);FindFirstObjectByType<QuestWorldStateReporter>()?.Revealed(target,i.containerId);state="revealed";}}break;}
        ...
        private static QuestInstance ToRuntime(ServerQuestInstanceDto setup)
        {
            ...
            var bindings=new List<QuestLockBinding>();foreach(var b in setup.key_lock_bindings??Array.Empty<ServerQuestBindingDto>())bindings.Add(new QuestLockBinding{lockId=QuestCanonicalIds.Normalize(b.lock_id),requiredKeyId=b.key_id,targetObjectId=b.role=="exit"?"door_001":"table_drawer_002"});
            ...
            return new QuestInstance{questId=setup.quest_instance_id,questSetId=QuestCanonicalSetIds.Normalize(setup.quest_set_id),placements=placements.ToArray(),lockBindings=bindings.ToArray(),notes=notes.ToArray(),initialStates=states.ToArray(),requiredRuntimeObjects=runtime.ToArray(),relevantObjectIds=setup.relevant_object_ids};
        }
    }
}
