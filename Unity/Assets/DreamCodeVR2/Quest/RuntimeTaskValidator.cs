...
namespace DreamCodeVR2.Quest
{
    public class RuntimeTaskValidator : MonoBehaviour
    {
        public bool ValidateNextTask(NextTaskSpec spec, out string error)
        {
            ...
            foreach(var condition in spec.successConditions??Array.Empty<RuntimeSuccessCondition>()){if(!IsAllowed(condition)){error="Task uses a non-allowlisted success condition.";return false;}if(!string.IsNullOrWhiteSpace(condition.object_id)&&AuthoringActionExecutor.FindEditable(condition.object_id)==null){error="Task references an unavailable runtime object: "+condition.object_id;DreamCodeVR2ClientLogger.Warn("c3","C3_TASK_ACTIVATION_REJECTED",error,new { task_id=spec.taskId,object_id=condition.object_id });return false;}if(string.Equals(condition.type,"DRAWER_DISCOVERY",StringComparison.OrdinalIgnoreCase)&&AuthoringActionExecutor.FindEditable(condition.container_id)==null){error="Task references an unavailable discovery container: "+condition.container_id;DreamCodeVR2ClientLogger.Warn("quest","FIXED_QUEST_DISCOVERY_CONTAINER_REJECTED",error,new { task_id=spec.taskId,container_id=condition.container_id });return false;}if(condition.type=="OBJECT_AT_ANCHOR"&&!HasAnchor(condition.anchor_id)){error="Task references an unavailable anchor: "+condition.anchor_id;DreamCodeVR2ClientLogger.Warn("c3","C3_TASK_ACTIVATION_REJECTED",error,new { task_id=spec.taskId,anchor_id=condition.anchor_id });return false;}} return true;
        }
        public bool IsSatisfied(RuntimeSuccessCondition condition,string taskId=null)
        {
            ...
            switch((condition.type??string.Empty).ToUpperInvariant())
            {
                ...
                case "DRAWER_DISCOVERY": return FindFirstObjectByType<QuestWorldStateReporter>()?.EvaluateDrawerDiscovery(taskId,condition.container_id,condition.object_id)==true;
        ...
        { if(c==null)return false; switch((c.type??string.Empty).ToUpperInvariant()){case "OBJECT_AT_ANCHOR":case "PAINTING_ALIGNED":case "OBJECT_REVEALED":case "OBJECT_HELD":case "OBJECT_OPEN":case "OBJECT_CLOSED":case "LOCK_UNLOCKED":case "OBJECT_ACTIVE":case "OBJECT_INACTIVE":case "DOOR_OPEN":case "DRAWER_DISCOVERY":case "AUTHORING_OBJECT_CREATED":case "AUTHORING_PROPERTY_SET":case "OBJECT_HAS_STATE":case "OBJECT_HAS_AFFORDANCE":case "OBJECT_GRABBED":case "OBJECT_USED_WITH":case "OBJECT_LINK_ACTIVE":case "OBJECT_BEHAVIOR_ACTIVE":case "SEQUENCE_COMPLETED":case "MULTIPLE_CONDITIONS_ALL":case "MULTIPLE_CONDITIONS_ANY":return true;default:return false;} }
    }
}
