...
namespace DreamCodeVR2.Quest
{
    public class DynamicStoryTaskController : MonoBehaviour
    {
        ...
        public bool ActivateNextTask(NextTaskSpec spec,out string error){Resolve();DreamCodeVR2ClientLogger.Event("c3","C3_ACTIVATION_REQUEST",null,new { task_id=spec?.taskId });if(!WaitingForNextTask){error="No next task is pending.";return false;}if(!validator){error="Runtime task validator is unavailable.";return false;}if(!validator.ValidateNextTask(spec,out error))return false;ActiveDynamicTask=spec;completedTaskId=null;WaitingForNextTask=false;runtimeState?.ActivateDynamicTask(new QuestTaskSpec{step=1,type="DynamicRuntimeTask",target=spec.requiredObjects!=null&&spec.requiredObjects.Length>0?spec.requiredObjects[0]:null,description=spec.playerInstruction,protectedDuringTask=spec.protectedObjects,allowedAuthoringOperations=spec.allowedAuthoringScope});ui?.SetStatus(spec.playerInstruction);protocol?.SendNextTaskAck(spec.taskId);DreamCodeVR2ClientLogger.Event("c3","TASK_ACTIVATED",null,new { task_id=spec.taskId });TryCompleteActiveTask();return true;}
