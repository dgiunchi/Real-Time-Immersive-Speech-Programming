...
MATERIALS: after creating a primitive, ALWAYS set its colour via its Renderer (renderer.material.color = ...). Objects created with GameObject.CreatePrimitive start with a material this build cannot render, so an object whose colour you never set may not display correctly. The runtime repairs anything you miss, but setting the colour is how you express the design. \
